...
const checksum = (grid) => {
  return sum(grid.map((line) => Math.max(...line) - Math.min(...line)))
}
...
console.log("part1 test", checksum(parse(test1)))
console.log("part1", checksum(parse(input)))

console.log("part2 test", sum(parse(test2).map(evenlyDividablePairsQuotient)))
console.log("part2", sum(parse(input).map(evenlyDividablePairsQuotient)))
