...
const pp = input.split("\n").map((line) => {
  const [counts, character, password] = line.split(" ")
  return {
    fromTo: counts.split("-").map(Number),
    character: character[0],
    password,
  }
})

function isValidPasswordPart1({ fromTo, character, password }) {
  const found = Array.from(password).filter((c) => c === character).length
  return fromTo[0] <= found && found <= fromTo[1]
}

function isValidPasswordPart2({ fromTo, character, password }) {
  const foundAt1 = password[fromTo[0] - 1] === character
  const foundAt2 = password[fromTo[1] - 1] === character
  return foundAt1 !== foundAt2
}

console.log("part1", pp.filter(isValidPasswordPart1).length)
console.log("part1", pp.filter(isValidPasswordPart2).length)
