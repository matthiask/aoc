...
const requiredFieldsPresent = (passport) => {
  return required.every((key) => Object.hasOwn(passport, key))
}

const fieldInRange = (field, a, b) => (passport) => {
  const number = Number(passport[field])
  return a <= number && number <= b
}

console.log("part1", passports.filter(requiredFieldsPresent).length)
console.log(
  "part2",
  passports
    .filter(requiredFieldsPresent)
    .filter(fieldInRange("byr", 1920, 2002))
    .filter(fieldInRange("iyr", 2010, 2020))
    .filter(fieldInRange("eyr", 2020, 2030))
    .filter((passport) => {
    ...
    .filter((passport) => passport.hcl.match(/^#[0-9a-f]{6}$/))
    .filter((passport) => passport.ecl.match(/^(amb|blu|brn|gry|grn|hzl|oth)$/))
    .filter((passport) => passport.pid.match(/^[0-9]{9}$/)).length,
)
